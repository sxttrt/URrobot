...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1, flag=False, pos=None,more_size_x= 20, more_size_y =0):
    """
    ...
    :param flag: Whether to restrict detection to a specific region (based on pos)
    :param pos: Region of interest as (x1, y1, x2, y2) to crop the images for grasp detection
    :param more_size_x: x size of grasp detection
    :param more_size_y: y size of grasp detection
    :return: list of Grasps
    """
    # 调试：打印输入图像信息
    print("原始图像尺寸:")
    print(
        f"q_img: {q_img.shape}  ang_img: {ang_img.shape}  width_img: {width_img.shape if width_img is not None else 'N/A'}")

    # 如果flag为True且pos有效，限制在pos区域内检测
    if flag ==1 :

        print(f"裁剪图像区域: {pos}")
        x1, y1, x2, y2 = pos  # 提取图像框的坐标
        q_img = q_img[y1-more_size_y:y2+more_size_y, x1-more_size_x:x2+more_size_x]  # 截取指定区域的q_img
        ang_img = ang_img[y1-more_size_y:y2+more_size_y, x1-more_size_x:x2+more_size_x]  # 截取指定区域的ang_img

        if width_img is not None:
            width_img = width_img[y1-more_size_y:y2+more_size_y, x1-more_size_x:x2+more_size_x]  # 截取指定区域的width_img
    ...
    cv2.imshow("Cropped Image", q_img)

    # 等待按键并关闭窗口
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    # 使用peak_local_max来检测抓取点
    print("开始寻找局部最大值（抓取点）...")
    local_max = peak_local_max(q_img, min_distance=30, threshold_abs=0.3, num_peaks=no_grasps)
    print(f"检测到的局部最大值数量: {len(local_max)}")

    grasps = []
    for grasp_point_array in local_max:
        grasp_point = tuple(grasp_point_array)
        print(f"检测到抓取点: {grasp_point}")

        grasp_angle = ang_img[grasp_point]
        print(f"抓取点角度: {grasp_angle}")
        ...
        if width_img is not None:
            ...
            g.width = g.length / 2
            print(f"抓取点的宽度: {g.width}, 长度: {g.length}")
...
#     :return: list of Grasps
#     """
#     local_max = peak_local_max(q_img, min_distance=30, threshold_abs=0.3, num_peaks=no_grasps) #128 220
#
#     grasps = []
#     for grasp_point_array in local_max:
#         grasp_point = tuple(grasp_point_array)
